...
const TracksPage: React.FC = () => {
  ...
  const [play, { sound }] = useSound('/sounds/haptic.mp3', { 
    ...
    soundEnabled: true,
    preload: true,
    onload: () => console.log("Sound loaded successfully on Tracks page"),
  });
  ...
  useEffect(() => {
    const playAudio = () => {
      try {
        play();
        console.log("Playing sound on Tracks page");
      } catch (error) {
        console.error("Failed to play audio on Tracks page:", error);
      }
    };
      ...
      <main>
        <div className="pt-20 pb-10 text-center">
          ...
          <p className="text-xl text-green-300 max-w-3xl mx-auto">
            Choose your path and build something revolutionary
          </p>
        </div>
        <Tracks />
